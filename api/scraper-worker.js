 ...
 * most up-to-date scraper) to run headlessly via ScraperAPI + JSDOM.
 * fabtcg.com blocks this VPS's IP outright (verified: even a bare curl gets
 * HTTP 403, identically to a real stealth-equipped Puppeteer Chrome), so
 * pages are fetched through ScraperAPI instead of a local browser.
 */
...
async function main() {
  ...
  if (!SCRAPER_API_KEY) { console.error('SCRAPER_API_KEY not set in environment.'); process.exit(1); }
